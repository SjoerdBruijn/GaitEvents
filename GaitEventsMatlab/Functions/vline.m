function lineHandles_v = vline(x, lineType, label, textPosition, axesHandle,linewidth)
...
% Acknowledgement:  It was based on vline() written by Brandon Kuczenski.
    ...
    %keyboard
    if (isempty(lineType) )
        lineHandles_v = plot(axesHandle, [x';x'], Ylimits);
    else
        lineHandles_v = plot(axesHandle, [x';x'], Ylimits, lineType);
    end

    if exist('linewidth','var') && ~isempty(lineType)
        lineHandles_v = plot(axesHandle, [x';x'], Ylimits, lineType,'linewidth',linewidth);
    elseif exist('linewidth','var')
        lineHandles_v = plot(axesHandle, [x';x'], Ylimits,'linewidth',linewidth);
    end
    ...
    if( ~isempty(label) )
        ...
        for k=1:length(x)
            ...
            set( textHandles(k), 'color', get(lineHandles_v(k), 'color') );
        end                            
    end
    ...
    set(lineHandles_v,'tag','vline','handlevisibility','off')
