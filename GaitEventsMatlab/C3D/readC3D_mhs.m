...
% Modified by Sjoerd Bruijn, march 2012, to allow for reading of longer
% files. 
...
%FullFileName = strcat(PATH, Patname, Patinfo, Fwfree1);
...
if fid == -1
    ...
    disp(['File: ',FileName,' could not be opened'])
    %uiwait(h)
    return
end
...
if NM>0
    tmpMKR = fread(fid,inf,reps,4*NAPVF*NC);   % note four bytes per analog channel data skipped

    % Reshape/resize/reorder data
    % First do the markers
    tmpMKR = reshape(tmpMKR,4,NM,length(tmpMKR)./(4*NM)) ;           % go from one long column to a 4xNMxNVF matrix
...
if NC > 0
    ...
    tmpANL = fread(fid,inf,reps,4*4*NM);   % note: four bytes per 3d data skipped

    % Reshape the analog data
    ANALOGdat = reshape(tmpANL,NC,length(tmpANL)/NC);
else
    ANALOGdat = 0;
end
